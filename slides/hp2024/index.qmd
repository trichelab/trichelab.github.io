...
Let $X_1, ..., X_{\lambda}$ be independent, identically distributed (iid) Poisson variables. 

If $Y = \sum^{\lambda}_{i=1}X_i$, then $Y \sim Pois(\lambda)$.

Given that Y is a sum of iid random variables, we can then state (as $\lambda$ grows) that
...
thus $Y \sim Pois(\lambda) \to N(\lambda, \lambda)$ as $\lambda \to \infty$.
