...
If $ X \sim Bin(n,p) $,
and $ \lim_{n \to \infty, \  p \to 0} np = \lambda $, 
where $ e^{-\lambda} = \lim_{n \to \infty}\left(1-\frac{\lambda}{n}\right)^n $,
then $ X \sim Pois(\lambda) $ and 
...
Pr(X=k|\lambda) &= 
  ...
  \class{fragment}{\left( \frac{\lambda^k}{k!} \right) 
                   \left[
                     ...
                     \equiv e^{-\lambda} 
                   \right]}\\[3px]
  \class{fragment}{\therefore}
                &
  \class{fragment}{Pr(X=k|\lambda)= \frac{\lambda^{k}e^{-\lambda}}{k!}.}
\end{align}
$$
