...
$$
\begin{align}
If              &\class{fragment}{X \sim Bin(n, p) and \lim_{n \to \infty, p \to 0} np = \lambda}\\[3px]
where           &\class{fragment}{e^{-\lambda}=\lim_{n\to\infty}\left(1-\frac{\lambda}{n}\right)^n}\\[3px]
then, since     &\class{fragment}{X \sim Pois(\lambda),}\\[3px] 
Pr(X=k|\lambda) &= 
