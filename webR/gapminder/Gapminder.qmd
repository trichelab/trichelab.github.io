...
<a href="https://swcarpentry.github.io/r-novice-gapminder/14-knitr-markdown.html">Software Carpentries knitr lesson</a>, so here's some examples of what you can
do with these tools, and why you might want to learn how to use them.

Note: You don't have to host your Rmarkdown documents! In fact, Rstudio has a 
lovely option where you can publish your Rmarkdown or Quarto documents directly 
to <a href="https://rpubs.com/ttriche/1011627">rpubs.com</a>. (Also: the linked
markdown document was a quickie and contains a number of Worst Practices...)

## What `rmarkdown` does

`knitr` calls the `rmarkdown` package to process the chunks of code and markdown
that you have asked it to knit.  This implies that we can test `rmarkdown` while
kicking the tires on other aspects of the rendering process. 

Below, I use the `volcano` dataset included with R to test how well `rmarkdown`
can render Rmarkdown from within Rmarkdown. If that sounds silly... well, it is.
Here's the `volcano` data (press play to plot it):
...
rmarkdown::render("markdown") # generates markdown.html
```

You can <a href="markdown.html">visit the HTML in your browser</a> if you like.
At the moment, it's not clear to me that `rmarkdown::render` does what it
says, the way that it says in the manual. Alas!
...
The Gapminder dataset you've been using is from a TED talk by Hans Rosling, 
...
income, and population size for countries around the world over many years. Hans
