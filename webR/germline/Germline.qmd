  ...
  html:
    ...
    code-links:
      - text: Quarto (source code)
        icon: github
        href: https://github.com/trichelab/trichelab.github.io/blob/gh-pages/webR/germline/Germline.qmd
engine: knitr
knitr: 
  opts_chunk: 
    collapse: true
...
For counts, such as pathogenic germline variants in leukemia, we can 
use Poisson models:
...
However, sometimes we are curious if a natural boundary exists in the data, 
suggesting that a more than one process is generating the observations.
We can use resampling and mixture models to probe this: 
