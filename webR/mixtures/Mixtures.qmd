...
A particularly vivid example comes from neurofibromatosis type 1, or NF1, and 
<a href="https://www.youtube.com/watch?v=V0Hp3HnS8M4">genetically identical twins who look nothing alike</a>.
...
Below, I have written out the math for a simple mixture model like the ones we
are fitting here. Most readers can skip it without missing too much. 


::: {.callout-tip collapse="true" title="Mixture models: the math"}
...
to maximize the probability of observing our actual data.

The joint probability $\prod_{i=1}^n (x; \theta)$ of observing the data $X$ 
given the proposed model parameters $\theta$ is referred to as the 
_likelihood_ $\mathcal{L}(X|\theta)$ of the data, with $n$ observations and 
parameters $\theta = (\theta_1, ..., \theta_K) = (\theta_1, \theta_2)$ for us.

$$ 
\mathcal{L}(X|\theta) = \prod_{i=1}^n \sum_{k=1}^k \pi_k N(x_i; \theta_k)
...
$\mathcal{L}(X_i|\theta_k)$ for all $X_i$ where $K_i = k$. It turns out that  
...
use `mixR` for the calculations.  More on the mathematical details and cute 
math tricks can be found at
...
Let's fit a mixture model to the expression of EVI1 from the _MECOM_ gene locus.
...
web browser to follow along (and test assumptions we have made in the analysis).
