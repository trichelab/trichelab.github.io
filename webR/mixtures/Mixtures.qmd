...
## Background: genetically unexplained phenotypic variability
...
oncogene or a tumor suppressor, depending on which isoform is expressed. We 
might ask whether there appear to be just two groups (high or low), and we can
use the uncertainty of the mixture fit to help us answer this question. 

```{webr-r}

# Be sure to run the preceding chunk!
classification <- MLL_EVI1_fit$comp.prob
rownames(classification) <- rownames(MLL) 
...
MLL$poorOutcome <- (apply(outcomeGroup, 1, which.max) < 2)
with(MLL, table(poorOutcome, EVI1high))
chisq.test(with(MLL, table(poorOutcome, EVI1high)))
...
This is where the UK Twins stuff goes. 
